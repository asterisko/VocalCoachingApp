...
{
    for (size_t i=0; i<mCanvases.size(); i++)
        at(i)->exit();
}
...
void ofxUITabbedCanvas::remove(ofxUICanvas *aTabCanvas)
{
    ...
    else if (static_cast<size_t>(mCurrentTab)>=mCanvases.size()) {
...
void ofxUITabbedCanvas::rebuildTabs()
{
    ...
    if (!mCanvases.empty()) {
        ...
        for (size_t i=0; i<toggles.size(); i++) {
            ...
            toggles.at(i)->setColorFill(getColorFill());
            toggles.at(i)->setColorBack(getColorBack());
            toggles.at(i)->setColorFillHighlight(getColorBack());
            toggles.at(i)->setColorOutline(getColorOutline());
            toggles.at(i)->setColorOutlineHighlight(getColorOutlineHighlight());
            toggles.at(i)->setColorPadded(getColorPadded());
            toggles.at(i)->setColorPaddedOutline(getColorOutline());
        }
    }
}
