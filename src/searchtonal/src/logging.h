...
#define LOG_FATAL	5	// fatal error (erro q pode abortar o programa)
...
#define LOG(loglevel,...) \
	...
	switch(loglevel){\
		case LOG_DEBUG: printf("DEBUG\t%d:%s\t%s\t",__LINE__,__FILE__,__FUNCTION__); printf(__VA_ARGS__); printf("\n");	break; \
